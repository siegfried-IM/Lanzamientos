"""
Genera dashboard_v2.html leyendo los Excel del directorio.
...
# Fuente IQVIA: MAYO_3 (completo, Ene-2025 a May-2026, con Pack)
# Antes eran X_MOLECULA_2 / X_ATC, ahora estos los reemplazan.
MAYO_DIR = HERE / "MAYO"
XMOL_FILE = MAYO_DIR / "MOLECULA_MAYO_3.xlsx"
XATC_FILE = MAYO_DIR / "ATC_MAYO_3.xlsx"

TEMPLATE_FILE = HERE / "dashboard_v2.html"
OUTPUT_FILE = HERE / "dashboard_v3_Mayo.html"

# Archivos MAYO_3 (los mismos que XMOL/XATC, mantenidos para sanity check vs legacy histórico)
MAYO_MOL_FILE = MAYO_DIR / "MOLECULA_MAYO_3.xlsx"
MAYO_ATC_FILE = MAYO_DIR / "ATC_MAYO_3.xlsx"
MAYO_QLICK_FILE = MAYO_DIR / "QLICK_VENTA_INTERNA_MAYO_1.xlsx"

# Legacy X_MOLECULA_2 / X_ATC quedaron desactivados (MAYO_3 los reemplaza con histórico
# completo). Se conservan en disco como respaldo histórico.
...
def build_iqvia_products(maestro, xmol, xatc, source, xmol_overlay=None, xatc_overlay=None):
    """source: 'LEGACY' o 'MAYO'.

    Si xmol_overlay / xatc_overlay están presentes (típicamente archivos MAYO_2
    con datos hasta el mes corriente), se usan para llenar los meses que el
    archivo principal no cubre. El overlay es a nivel PRODUCTO (sin Pack).
    """
    ...
    # --- Overlay: meses que NO están en el archivo principal ---
    overlay_mol_months = []
    overlay_atc_months = []
    xmol_overlay_real = None
    xatc_overlay_real = None
    if xmol_overlay is not None:
        main_meses_es = {col_to_es_mes(c) for c in mol_months}
        overlay_mol_months = [c for c in xmol_overlay.columns if "Units" in str(c)
                              and col_to_es_mes(c) not in main_meses_es
                              and col_to_es_mes(c) in ALL_MONTHS]
        xmol_overlay_real = xmol_overlay[xmol_overlay["Manufacturer"] != "Grand Total"].copy()
    if xatc_overlay is not None:
        main_meses_es = {col_to_es_mes(c) for c in atc_months}
        overlay_atc_months = [c for c in xatc_overlay.columns if "Units" in str(c)
                              and col_to_es_mes(c) not in main_meses_es
                              and col_to_es_mes(c) in ALL_MONTHS]
        xatc_overlay_real = xatc_overlay[xatc_overlay["Manufacturer"] != "Grand Total"].copy()

    if overlay_mol_months:
        print(f"  Overlay MOL: meses extra = {[col_to_es_mes(c) for c in overlay_mol_months]}")
    if overlay_atc_months:
        print(f"  Overlay ATC: meses extra = {[col_to_es_mes(c) for c in overlay_atc_months]}")
    ...
    for iqv_name, grupo in mae.groupby("NOMBRE_IQUVIA", sort=False):
        ...
        # OVERLAY MAYO_2 sobre LEGACY (meses extra a nivel producto)
        si_overlay = {}
        if xmol_overlay_real is not None and overlay_mol_months:
            mol_ov = xmol_overlay_real[
                xmol_overlay_real["Product"].astype(str).str.strip() == iqv_name_clean
            ]
            if not mol_ov.empty:
                si_overlay = units_by_month(mol_ov.iloc[0], overlay_mol_months)
                # Capturar molecula/atc desde overlay si LEGACY no la tiene
                if molecula is None:
                    molecula = str(mol_ov.iloc[0]["Molecule"]).strip()
        if xatc_overlay_real is not None and atc is None:
            atc_ov = xatc_overlay_real[
                xatc_overlay_real["Product"].astype(str).str.strip() == iqv_name_clean
            ]
            if not atc_ov.empty:
                atc = str(atc_ov.iloc[0]["ATC-4"]).strip()
        ...
        for idx, (_, mrow) in enumerate(grupo.iterrows()):
            ...
            # Inyectar overlay (May-2026 al nivel producto) en la primera presentación
            if idx == 0 and si_overlay:
                for m, v in si_overlay.items():
                    si_dict[m] = v
        ...
        if molecula:
            ...
            # Overlay del mercado para meses extra (May-2026)
            if xmol_overlay_real is not None and overlay_mol_months:
                overlay_market = market_by_month(
                    xmol_overlay_real,
                    xmol_overlay_real["Molecule"].astype(str).str.strip() == molecula,
                    overlay_mol_months,
                )
                mercado_molecula.update(overlay_market)
        mercado_atc = {}
        if atc:
            ...
            # Overlay del mercado ATC para meses extra (May-2026)
            if xatc_overlay_real is not None and overlay_atc_months:
                overlay_market = market_by_month(
                    xatc_overlay_real,
                    xatc_overlay_real["ATC-4"].astype(str).str.strip() == atc,
                    overlay_atc_months,
                )
                mercado_atc.update(overlay_market)
...
def main():
    ...
    print(f"  X_MOL:   {xmol.shape}  (fuente: {XMOL_FILE.name})")
    print(f"  X_ATC:   {xatc.shape}  (fuente: {XATC_FILE.name})")
    if ventas_mayo is not None:
    ...
    data = {
        ...
        "meta": {
            ...
            "latest_iqvia": "May-2026",
            "latest_qlick": "May-2026",
        },
    }
    ...
    # Sanity check vs MAYO sólo si XMOL no es ya un archivo MAYO
    if source == "LEGACY" and "MAYO" not in XMOL_FILE.name.upper():
